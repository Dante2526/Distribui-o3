...
export const generatePdfBlob = async (data: PdfReportData): Promise<Blob> => {
  ...
  pdf.text(
    `RESUMO GERAL - TURMA ${data.turma} - ${data.dataFormatada}`,
    pageWidth / 2,
    18,
    { align: "center" },
  );
  ...
  const drawSection = (
    ...
    const statusGroups: {
      ...
      {
        key: "BEM",
        label: 'STATUS: "ASS.DSS + ESTOU BEM"',
        emps: emps.filter((e) => e.s === "BEM"),
      },
      {
        key: "MAL",
        label: 'STATUS "ESTOU MAL"',
        emps: emps.filter((e) => e.s === "MAL"),
      },
      {
        key: "PEN",
        label: "PENDENTES",
        emps: emps.filter((e) => e.s === "PEN"),
      },
      {
        key: "AUS",
        label: "AUSENTES",
        emps: emps.filter((e) => e.s === "AUS"),
      },
    ];
  ...
  const drawRegistros = (
    ...
    if (registros.length === 0) {
      ...
      pdf.setTextColor(120, 130, 145);
      pdf.text(
        `Nenhum registro de assunto encontrado para ${turnoLabel}.`,
        margin + 2,
        y,
      );
      y += 6;
      ...
      registros.forEach((reg) => {
        checkPageBreak(14);
        const nameText = reg.name
          ? `${reg.name} (Matrícula: ${reg.matricula || "---"})`
          : "Nome não informado";
        pdf.setFont("helvetica", "bold");
        pdf.setFontSize(9);
        pdf.setTextColor(30, 41, 59);
        ...
        const assuntoLines = pdf.splitTextToSize(
          `Assunto: ${reg.assunto || "NÃO PREENCHIDO"}`,
          contentWidth - 8,
        );
  ...
  pdf.text(
    `Gerado em ${new Date().toLocaleString("pt-BR")} — Painel DSS`,
    pageWidth / 2,
    footerY,
    { align: "center" },
  );
...
export const exportToPdf = async (
  _elementIdOrData: string | PdfReportData,
  filename: string,
...
const createSheetForReport = (data: PdfReportData): XLSX.WorkSheet => {
  ...
  if (data.turma !== "C_CG") {
    ...
    if (
      secTeam.length > 0 ||
      (data.registros6H && data.registros6H.length > 0)
    ) {
...
export const generateExcelBlob = (
  ...
  try {
    ...
    const isPdfReportDataArray =
      Array.isArray(dataOrArray) &&
      dataOrArray.length > 0 &&
      typeof dataOrArray[0] === "object" &&
      "turma" in dataOrArray[0] &&
      "employees" in dataOrArray[0];
    ...
    if (isPdfReportDataArray) {
      ...
      ws["!cols"] = [{ wpx: 200 }, { wpx: 100 }, { wpx: 100 }, { wpx: 150 }];
      XLSX.utils.book_append_sheet(wb, ws, "Relatório");
    }
