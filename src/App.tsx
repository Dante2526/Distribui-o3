...
const deduplicateAnnotationItems = (groups: AnnotationGroup[]): AnnotationGroup[] => {
...
function AppContent() {
    ...
    const unsubscribe = firestoreService.subscribeToBoardState(selectedTurma, (state) => {
      ...
      if (state && state.departmentsData && state.departmentsData.length > 0 && hasAnyEmployee) {
        ...
        if (state.annotationsLeft && state.annotationsLeft.length > 0) {
          setAnnotationsLeft(deduplicateAnnotationItems(state.annotationsLeft));
        }
        if (state.annotationsRight && state.annotationsRight.length > 0) {
          setAnnotationsRight(deduplicateAnnotationItems(state.annotationsRight));
        }
  ...
  const pendingFieldLogsRef = useRef<Record<string, { oldValue: string, timeoutId: ReturnType<typeof setTimeout> | null }>>({});
  ...
  const handleClearAll = useCallback(() => {
    ...
    setSpecialShiftData(prev => prev.map(emp => ({ ...emp, line: '', machine: '' })));
    setSupportRolesData(prev => prev.map(group => group.map(emp => ({ ...emp, role: '' }))));
  ...
  const handleGenerateReport = useCallback(() => {
    ...
    departmentsData.forEach(d => {
      ...
      d.data.forEach(e => {
        if (e.name.trim()) report += `  - ${e.name} (Matrícula: ${e.matricula || 'S/N'}, Linha: ${e.line || '---'}, Loco: ${e.machine || '---'})\n`;
      });
    });
    ...
    setReportStats({
      ...
      fora: totalFora,
      atm: totalATM,
      restricao: todasAnotacoes.filter(item => item.status.toUpperCase().includes('RESTRI') || item.status.toUpperCase().includes('RESTRICAO')).length,
      estagio: todasAnotacoes.filter(item => item.status.toUpperCase().includes('ESTÁGIO') || item.status.toUpperCase().includes('ESTAGIO')).length,
      inss: todasAnotacoes.filter(item => item.status.toUpperCase() === 'INSS').length,
      treinamento: todasAnotacoes.filter(item => item.status.toUpperCase().includes('TREINA')).length,
      revezamento: todasAnotacoes.filter(item => item.status.toUpperCase().includes('REVEZA')).length,
    });
  ...
  const dragSourceRef = useRef<{ id: string; type: 'maquinista' | 'apoio' | 'special'; originalContainer: string; originalRole?: string } | null>(null);
  ...
  const touchSensor = useSensor(TouchSensor, React.useMemo(() => ({
    activationConstraint: { delay: isAdmin ? 250 : 999999, tolerance: isAdmin ? 5 : 0 },
  }), [isAdmin]));
  ...
  const handleDragStart = useCallback((event: any) => {
    ...
    if (!sourceContainer) {
      for (let idx = 0; idx < supportRolesDataRef.current.length; idx++) {
        const emp = supportRolesDataRef.current[idx].find(e => e.id === activeIdVal);
        if (emp) {
  ...
  const handleDragCancel = useCallback(() => {
    ...
    if (clonedDepartmentsRef.current) setDepartmentsData(clonedDepartmentsRef.current);
    if (clonedSupportRef.current) setSupportRolesData(clonedSupportRef.current);
    if (clonedSpecialShiftRef.current) setSpecialShiftData(clonedSpecialShiftRef.current);
  }, []);
  ...
  const handleDragOver = useCallback((event: any) => {
    ...
    if (!activeType) {
      const idx = specialShiftDataRef.current.findIndex(e => e.id === activeId);
      if (idx !== -1) {
    ...
    if (overId === 'recepcao' || overId === 'classificacao' || overId === 'formacao') {
      ...
      if (!overContainer) {
        const specialIdx = specialShiftDataRef.current.findIndex(e => e.id === overId);
        if (specialIdx !== -1) {
          overContainer = 'special-shift';
          overType = 'special';
          overIdx = specialIdx;
    ...
    if (activeContainer === overContainer) {
      if (activeType === 'maquinista') {
        ...
        const activeIndex = specialShiftDataRef.current.findIndex(e => e.id === activeId);
        const overIndex = overIdx >= 0 ? overIdx : specialShiftDataRef.current.length - 1;
        if (activeIndex !== overIndex && activeIndex !== -1) {
          setSpecialShiftData(prev => arrayMove(prev, activeIndex, overIndex));
        }
      }
      return;
    }
    ...
    // G: Maquinista -> Special
      ...
      const adaptedSpecial: Employee = {
        ...
        originalSupportRole: isOriginallyApoio ? (dragSourceRef.current?.originalRole || 'VIRADOR') : undefined
      };
      ...
      setSupportRolesData(prev => prev.map(group => group.filter(e => e.id !== activeId)));
    ...
    if (over && activeIdVal) {
      ...
      if (!initialTitle && clonedSupportRef.current) {
        ...
        for (let i = 0; i < clonedSupportRef.current.length; i++) {
          const emp = clonedSupportRef.current[i].find(e => e.id === activeIdVal);
          if (emp) { 
      ...
      if (!initialTitle && clonedSpecialShiftRef.current) {
        const emp = clonedSpecialShiftRef.current.find(e => e.id === activeIdVal);
        if (emp) {
      ...
      let finalTitle = '';
      for (const dept of departmentsDataRef.current) {
        if (dept.data.some(e => e.id === activeIdVal)) { finalTitle = dept.title; break; }
      }
      if (!finalTitle) {
        const supportTitles = ["Recepção", "Classificação", "Formação"];
        for (let i = 0; i < supportRolesDataRef.current.length; i++) {
          if (supportRolesDataRef.current[i].some(e => e.id === activeIdVal)) {
            finalTitle = `Apoio - ${supportTitles[i] || `Grupo ${i+1}`}`; break;
          }
        }
      }
      if (!finalTitle) {
  ...
  const { activeEmployee, activeDepartment, activeSupportItem, activeSupportGroupIndex, activeSpecialEmployee } = useMemo(() => {
    const emp = activeId ? departmentsData.flatMap(d => d.data).find(e => e.id === activeId) : null;
    const dept = emp ? departmentsData.find(d => d.data.some(e => e.id === activeId)) : null;
    const supportItem = activeId ? supportRolesData.flatMap(g => g).find(e => e.id === activeId) : null;
    const supportGroupIndex = supportItem ? supportRolesData.findIndex(g => g.some(e => e.id === activeId)) : -1;
    const specialEmp = activeId ? specialShiftData.find(e => e.id === activeId) : null;
    return { 
      activeEmployee: emp, 
  ...
  const handleAdminLoginError = useCallback(() => {
    ...
    if (errorToastTimerRef.current) clearTimeout(errorToastTimerRef.current);
    errorToastTimerRef.current = setTimeout(() => setShowErrorToast(false), 3500);
  }, []);
  ...
  const handleUpdateSpecialShiftEmployee = useCallback((empIndex: number, field: keyof Employee, value: any) => {
    setSpecialShiftData(prev => {
      ...
      if ((field === 'line' || field === 'machine') && empName && oldValue !== value) {
        ...
        pendingFieldLogsRef.current[logKey].timeoutId = setTimeout(() => {
          ...
          if (originalOldValue !== value) {
            logMovement(empName, fromStr, toStr, field === 'line' ? value : undefined, field === 'machine' ? value : undefined);
          }
  ...
  const handleTransferToSpecialShift = useCallback((sourceDeptId: string, sourceEmpIndex: number) => {
    const sourceDept = departmentsDataRef.current.find(d => d.id === sourceDeptId);
  ...
  const handleMarkEmployeeAbsent = useCallback((
    ...
    else if (absenceType === 'FORA' || absenceType === 'ATM') targetLeftGroupIndex = 2;
    else if (absenceType === 'RESTRIÇÃO') targetRightGroupIndex = 1;
    else if (absenceType === 'INSS') targetRightGroupIndex = 2;
    else if (absenceType === 'TREINAMENTO' || absenceType === 'REVEZAMENTO') targetLeftGroupIndex = 0;
  ...
  const handleReturnFromAnnotation = useCallback((isLeft: boolean, groupIdx: number, itemIdx: number) => {
    ...
    if ((item as any).originalSupportGroupIndex !== undefined) {
      ...
      logMovement(item.name, item.status, groupName, undefined, item.matricula);
      ...
      const dept = departmentsDataRef.current?.find(d => d.id === item.originalDeptId);
      if (dept) deptName = dept.title;

      logMovement(item.name, item.status, deptName, undefined, item.matricula);
  ...
  const {
    ...
    const tMaquinistas = departmentsData.reduce((acc, dept) => acc + dept.data.filter(emp => emp.name.trim() !== '').length, 0);
    const tApoio = supportRolesData.reduce((acc, group) => acc + group.filter(emp => emp.name.trim() !== '').length, 0);
    const tTurno6H = specialShiftData.filter(emp => emp.name.trim() !== '').length;
    ...
    return {
      ...
      totalFerias: todasAnotacoes.filter(item => item.status.toUpperCase().includes('FÉRIA') || item.status.toUpperCase().includes('FERIA')).length,
      totalFora: todasAnotacoes.filter(item => item.status.toUpperCase() === 'FORA').length,
      totalATM: todasAnotacoes.filter(item => 
      ...
      totalRestricao: todasAnotacoes.filter(item => 
        item.status.toUpperCase().includes('RESTRI') || 
        item.status.toUpperCase().includes('RESTRICAO')
      ).length,
      totalEstagio: todasAnotacoes.filter(item => 
        item.status.toUpperCase().includes('ESTÁGIO') || 
        item.status.toUpperCase().includes('ESTAGIO')
      ).length,
      totalINSS: todasAnotacoes.filter(item => item.status.toUpperCase() === 'INSS').length,
      totalTreinamento: todasAnotacoes.filter(item => item.status.toUpperCase().includes('TREINA')).length,
      totalRevezamento: todasAnotacoes.filter(item => item.status.toUpperCase().includes('REVEZA')).length
    };
  }, [departmentsData, supportRolesData, specialShiftData, annotationsLeft, annotationsRight]);
        ...
        {activePage === 'home' ? (
          <div ref={viewportRef} className={`viewport absolute inset-0 ${isDarkMode ? 'bg-[#111217]' : 'bg-[#eef2f7]'}`}>
            <div ref={contentWrapperRef} className="origin-top-left overflow-hidden">
              <div ref={scalableContainerRef} className="scalable-container w-fit origin-top-left p-8 pb-[300px]">
              ...
              <div className="flex flex-col items-end gap-11">
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    style={{ width: '190px', height: '90px', borderRadius: '99em', color: '#ffffff' }}
                  >
            ...
            >
              ...
              <SpecialShiftContainer is6HActive={is6HActive} isEmpty={specialShiftData.length === 0}>
                {specialShiftData.length === 0 ? (
                  <div className="flex items-center justify-center gap-4 py-2 px-6 w-full">
                    ...
                    <div>
                      <h2 className="text-xl font-bold text-white tracking-wide uppercase leading-none">TURNO 6H</h2>
                    </div>
                  </div>
                ) : (
                  <>
