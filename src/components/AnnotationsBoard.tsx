...
const AnnotationItemRow = React.memo(
  ({
    ...
    return (
      <div
        className={`annotation-item-row flex items-center justify-between px-2.5 py-2 bg-[#111217] rounded-[8px] border border-white/[0.03] gap-2 transition-all duration-300 md:hover:-translate-y-1 md:hover:bg-[#252836] md:hover:border-[#FF9F0A]/30 md:hover:shadow-md group ${isAnimating ? "animate-border-spin" : ""}`}
      >
        <div className="flex flex-col gap-0.5 w-[60%] min-w-0">
          <input
            ...
            value={item.name}
            onChange={(e) =>
              onUpdate(groupIdx, itemIdx, "name", e.target.value)
            }
            readOnly={!isAdmin}
          ...
          <div className="flex items-center gap-1">
            ...
            <input
              ...
              onChange={(e) =>
                onUpdate(groupIdx, itemIdx, "matricula", e.target.value)
              }
              readOnly={!isAdmin}
              placeholder="N/A"
              maxLength={8}
        ...
        <input
          ...
          value={item.status}
          onChange={(e) =>
            onUpdate(groupIdx, itemIdx, "status", e.target.value)
          }
          readOnly={!isAdmin}
          placeholder="STATUS"
...
export const AnnotationsBoard = React.memo(
  ({
    ...
    const handleReturn = useCallback(
      (side: "left" | "right", groupIdx: number, itemIdx: number) => {
      ...
      <div className="annotations-board-panel bg-[#1E2029] rounded-[24px] overflow-hidden flex flex-col shadow-lg border border-white/[0.02] min-h-full">
        ...
        <div className="px-6 py-5 border-b border-[#FF9F0A]/20 flex items-center justify-center bg-gradient-to-r from-[#FF9F0A]/10 to-[#FF6B00]/5 bg-[#15171E] relative">
          ...
          <h3 className="text-[22px] font-extrabold text-[#FF9F0A] tracking-wider uppercase drop-shadow-sm flex items-center gap-2.5">
            <FileText
              className="w-[22px] h-[22px] text-[#FF9F0A]"
              strokeWidth={2.5}
            />
            ANOTAÇÕES MINÉRIO
          </h3>
