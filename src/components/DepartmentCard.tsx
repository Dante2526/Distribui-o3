...
export const DepartmentCard = React.memo(
  ({
    ...
    const wrappedOnMarkAbsent = React.useCallback(
      (empIndex: number, absenceType: StatusType) =>
        onMarkAbsent(department.id, empIndex, absenceType),
    ...
    const sortableItems = React.useMemo(
      () => department.data.map((e) => e.id || e.name),
      [department.data],
    );
      ...
      <div className="dept-card-panel bg-[#1E2029] rounded-[24px] overflow-hidden flex flex-col shadow-lg border border-white/[0.02] min-h-full">
        ...
        <div className="px-6 py-5 border-b border-[#111217] flex items-center justify-between bg-[#15171E]">
          <div className="flex items-center gap-4">
            <div
              className={`w-12 h-12 rounded-[14px] flex items-center justify-center shadow-inner ${theme.bg} ${theme.color}`}
            >
              <DeptIcon
                iconName={theme.iconName as string}
                className="w-6 h-6"
              />
            </div>
            <h3 className="text-[22px] font-bold text-white tracking-tight uppercase">
              {department.title}
            </h3>
          </div>
          <div
        ...
        <div
          ref={setNodeRef}
          className="flex-1 p-5 bg-[#0D0E12]/30 flex flex-col"
        >
          <SortableContext
            id={department.id}
            items={sortableItems}
          ...
          >
            <div
              className="grid grid-cols-1 gap-4 flex-1 content-start"
              style={{ minHeight: `${maxCount * 156}px` }}
            >
