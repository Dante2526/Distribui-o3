...
import { dbDSS as db } from "../../lib/firebase";
import { luminaDb } from "../../lib/luminaFirebase";
import {
  doc,
  getDoc,
  collection,
...
const HistoryModal: React.FC<{
  ...
  const [luminaEvents, setLuminaEvents] = useState<
    { id: string; title: string; category: string; timeLeft: number }[]
  >([]);
  const [loadingLuminaEvents, setLoadingLuminaEvents] = useState(false);
  const [selectedLuminaEventId, setSelectedLuminaEventId] = useState<
    string | null
  >(null);
  ...
  const handleSendToLumina = async () => {
    ...
    try {
      ...
      showNotification(
        `Evidência enviada para o Lumina (${luminaClassId})!`,
        "success",
      );
  ...
  const [selectedSearchTurmas, setSelectedSearchTurmas] = useState<string[]>(
    turma ? [turma] : [],
  );
  ...
  const loadHistoryBatch = async (
    isManualLoadMore = false,
    turmasParaBusca?: string[],
  ) => {
    const turmaFilter = turmasParaBusca ?? (turma ? [turma] : []);
    if (turmaFilter.length === 0 || !db || (fetchingMore && isManualLoadMore))
      return;
    ...
    try {
      ...
      if (snapshot.empty) {
        ...
        setAllRecords((prev) =>
          isManualLoadMore ? [...prev, ...newRecords] : newRecords,
        );
  ...
  React.useEffect(() => {
    if (debouncedSearch.trim() && hasMore && !fetchingMore) {
  ...
  const generateRecordText = (record: HistoryRecord) => {
    const dataExibicao = (record.data || "")
      .split("/")
      .map((s, i) => (i === 2 && s.length > 2 ? s.slice(-2) : s))
      .join("/");
    ...
    if (record.registros7H && record.registros7H.length > 0) {
      record.registros7H.forEach((reg) => {
        ...
        if (reg.name)
          report += `  Responsável: ${reg.name} (Matrícula: ${reg.matricula || "---"})\n`;
      });
    } else {
  ...
  const handleExportAllZip = async (
    format: "TXT" | "PDF" | "DOC" | "EXCEL",
  ) => {
    const recordsToDownload =
      selectedRecordsToExport.length > 0
        ? filteredResults.filter((rec) =>
            selectedRecordsToExport.includes(`${rec.turma}_${rec.dataISO}`),
          )
        : filteredResults;
    ...
    showNotification(
      format === "EXCEL"
        ? `Gerando planilha agrupada, aguarde...`
        : `Gerando arquivos ${format}, aguarde...`,
      "success",
    );
    ...
    try {
      ...
      if (filesToZip.length === 1) {
        ...
        const fileBlob =
          typeof singleFile.content === "string"
            ? new Blob([singleFile.content], {
        ...
        <div className="space-y-2">
          {Object.entries(grouped).map(([status, emps]) => {
            ...
            return (
              ...
              >
                <div
                  className={`flex items-center gap-1.5 mb-2 ${config.color} font-bold text-[10px] uppercase tracking-wide`}
                >
                  <span
                    className={`w-2 h-2 rounded-full ${STATUS_DOT_COLORS[status]} ${status === "MAL" ? "animate-pulse" : ""}`}
                  ></span>
  ...
  return (
    ...
    >
      <div className="w-full">
          ...
          <div className="space-y-3 mb-6 animate-in fade-in zoom-in-95 duration-200">
                      ...
                      {isExportMenuOpen && (
                        <div className="absolute top-full mt-2 right-0 w-48 bg-white dark:bg-dark-card rounded-xl shadow-xl border border-gray-200 dark:border-white/10 overflow-hidden z-50 origin-top animate-in fade-in slide-in-from-top-2 duration-200">
                          <div className="flex flex-col py-1">
                            <button
                              onClick={() => handleExportAllZip("PDF")}
                            ...
                            <button
                              onClick={() => handleExportAllZip("EXCEL")}
                              className="px-4 py-3 text-left text-sm font-semibold flex items-center gap-3 hover:bg-gray-100 dark:hover:bg-dark-hover text-gray-800 dark:text-gray-200 transition-colors"
                            >
                            ...
                            <button
                              onClick={() => handleExportAllZip("DOC")}
                              className="px-4 py-3 text-left text-sm font-semibold flex items-center gap-3 hover:bg-gray-100 dark:hover:bg-dark-hover text-gray-800 dark:text-gray-200 transition-colors"
                            >
                            ...
                            <div className="h-px bg-gray-200 dark:bg-dark-hover my-1 mx-2"></div>
                            <button
                              onClick={() => handleExportAllZip("TXT")}
                              className="px-4 py-3 text-left text-sm font-semibold flex items-center gap-3 hover:bg-gray-100 dark:hover:bg-dark-hover text-gray-800 dark:text-gray-200 transition-colors"
                            >
            ...
            <div className="grid grid-cols-1 gap-2 max-h-[300px] overflow-y-auto pr-1 custom-scrollbar">
              {filteredResults.map((rec) => {
                ...
                const match7H = rec.registros7H?.some((r) =>
                  isMatch(r.assunto),
                );
                const match6H = rec.registros6H?.some((r) =>
                  isMatch(r.assunto),
                );
                const matchedAssunto =
                  rec.registros7H?.find((r) => isMatch(r.assunto))?.assunto ||
                  rec.registros6H?.find((r) => isMatch(r.assunto))?.assunto ||
                  rec.registros7H?.[0]?.assunto ||
                  rec.registros6H?.[0]?.assunto ||
                  "Tema não preenchido";
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <div
                        className={`w-5 h-5 rounded flex items-center justify-center border-2 transition-colors ${isSelected ? "bg-indigo-500 border-indigo-500 text-white" : "border-gray-300 dark:border-gray-600 group-hover:border-indigo-400"}`}
                      >
                        {isSelected && (
                          <svg
              ...
              {!isSearching && filteredResults.length === 0 && (
                <div className="text-center py-8 bg-gray-50 dark:bg-gray-800/30 rounded-2xl border border-dashed border-gray-200 dark:border-gray-700">
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    Nenhum tema encontrado neste período.
                  </p>
                </div>
              )}
        ...
        {historyData && !loading && (
          <div
            id="history-capture-area"
            className="space-y-4 bg-light-card dark:bg-dark-card pt-1 px-4"
          >
            {/* Data formatada */}
            ...
            <div className="flex gap-3">
              ...
              <div className="flex-1 bg-blue-50 dark:bg-blue-900/20 p-3 rounded-xl border border-blue-100 dark:border-blue-800 text-left relative overflow-hidden group">
                ...
                <div className="text-[10px] font-bold text-white bg-blue-500 px-2 py-0.5 rounded-full w-fit mb-2">
                  TURNO {mainShiftLabel}
                </div>
                <div className="mb-2 relative z-10">
                  <span className="text-[9px] uppercase text-gray-500 dark:text-gray-400 block font-bold">
                    Tema DSS
                  </span>
              ...
              {turma !== "C_CG" && (
                <div className="flex-1 bg-orange-50 dark:bg-orange-900/20 p-3 rounded-xl border border-orange-100 dark:border-orange-800 text-left relative overflow-hidden group">
                  ...
                  <div className="text-[10px] font-bold text-white bg-orange-500 px-2 py-0.5 rounded-full w-fit mb-2">
                    TURNO {shiftLabel}
                  </div>
                  <div className="mb-2 relative z-10">
                    <span className="text-[9px] uppercase text-gray-500 dark:text-gray-400 block font-bold">
            ...
            <div className="grid grid-cols-4 gap-2">
              <div className="bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-lg p-2 flex flex-col items-center justify-center shadow-sm">
                <span className="text-xl font-bold text-green-600 dark:text-green-400">
                  {historyData.totalPresentes}
                </span>
                <span className="text-[8px] uppercase text-gray-500 font-bold tracking-tight">
                  Presentes
                </span>
              </div>
              <div className="bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-900/30 rounded-lg p-2 flex flex-col items-center justify-center shadow-sm">
                <span className="text-xl font-bold text-red-600 dark:text-red-400">
                  {historyData.totalMal}
                </span>
                <span className="text-[8px] uppercase text-red-500/80 font-bold tracking-tight">
                  Mal
                </span>
              </div>
              <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-900/30 rounded-lg p-2 flex flex-col items-center justify-center shadow-sm">
                <span className="text-xl font-bold text-amber-600 dark:text-amber-400">
                  {historyData.totalAusentes}
                </span>
                <span className="text-[8px] uppercase text-amber-500/80 font-bold tracking-tight">
                  Ausentes
                </span>
              </div>
                ...
                {(() => {
                  ...
                  return (
                    <ExportDropdown
                      label={
                        isMultiSelect
                          ? selectedRecordsToExport.length === 1
                            ? "BAIXAR SELECIONADO"
                            : "BAIXAR TODOS"
                          : "BAIXAR"
                      }
                      onExportTxt={
                        isMultiSelect
                          ? () => handleExportAllZip("TXT")
                          : handleExportTxt
                      }
                      onExportPng={handleExportPng} // PNG só funciona pro DOM atual
                      onExportPdf={
                        isMultiSelect
                          ? () => handleExportAllZip("PDF")
                          : handleExportPdf
                      }
                      onExportDoc={
                        isMultiSelect
                          ? () => handleExportAllZip("DOC")
                          : handleExportDoc
                      }
                      onExportExcel={
                        isMultiSelect
                          ? () => handleExportAllZip("EXCEL")
                          : handleExportExcel
                      }
                ...
                <div className="bg-gradient-to-r from-violet-600 to-indigo-600 px-4 py-3 flex items-center justify-between">
                  <div>
                    <p className="text-white font-bold text-sm">
                      Selecione o Evento no Lumina
                    </p>
                    <p className="text-violet-200 text-[10px] mt-0.5">
                      Evidência será enviada para:{" "}
                      <strong>{luminaClassId}</strong>
                    </p>
                  </div>
                  <button
                    onClick={() => {
                ...
                <div className="p-3 bg-violet-50 dark:bg-violet-950/30 space-y-2 max-h-52 overflow-y-auto">
                  {loadingLuminaEvents ? (
                    ...
                    <p className="text-center text-sm text-gray-500 py-4">
                      Nenhum evento ativo encontrado no Lumina.
                    </p>
                  ) : (
                    luminaEvents.map((ev) => {
